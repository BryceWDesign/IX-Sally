...
def test_runtime_baseline_uses_canonical_version() -> None:
    ...
    assert (
        session_one_baseline_payload()["version"]
        == __version__
    )
...
def test_project_metadata_uses_dynamic_version_source() -> None:
    ...
    assert project_data["project"]["dynamic"] == [
        "version"
    ]
    assert (
        project_data["tool"]["setuptools"]["dynamic"]["version"]
        == {
...
def test_package_smoke_reads_canonical_source_version() -> None:
    ...
    assert package_smoke._source_version(
