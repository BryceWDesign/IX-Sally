...
def _state() -> NinefoldRunState:
    contract = AutonomyContract.create(
        goal="Record human-review reentry audit reports in the control plane.",
...
def _review_action(
    description: str = "Run audited control-plane reentry verification.",
) -> BoundedActionRecord:
...
def _audit_report(max_steps: int = 1):
...
def test_control_plane_coordinator_records_reentry_audit_operation() -> None:
    audit_report = _audit_report()

    result = HumanReviewControlPlaneCoordinator.create().record_reentry_audit(
    ...
    assert result.after_control_plane.latest_reentry_audit_digest() == (
        result.after_control_plane.reentry_audit_ledger.latest().digest().value
    )
    assert result.require_reentry_audit_report() == audit_report
    assert result.receipt.reentry_audit_count == 1


def test_control_plane_reentry_audit_operation_payload_links_report_digest() -> None:
    audit_report = _audit_report()
    control_plane = audit_report_to_control_plane(audit_report)
    ...
    assert payload["reentry_audit_report_digest"] == audit_report.digest().value
    assert payload["reentry_count"] == 1


def test_control_plane_reentry_audit_operation_rejects_mismatched_control_plane() -> None:
    audit_report = _audit_report()

    with pytest.raises(FoundationError, match="reentry audit must match current"):
        HumanReviewControlPlaneCoordinator.create().record_reentry_audit(
            audit_report=audit_report,
            control_plane=HumanReviewControlPlaneState.create(),
        )


def test_control_plane_operation_result_requires_reentry_audit_report() -> None:
    run_state = _state().with_action(_review_action())
...
def test_control_plane_operation_receipt_rejects_negative_reentry_audit_count() -> None:
    digest = DigestRecord.from_payload(
        {"record": "control-plane-reentry-audit-operation"},
    )

    with pytest.raises(FoundationError, match="reentry_audit_count must not be negative"):
...
def audit_report_to_control_plane(audit_report):
    """Return the control plane that produced the audit report."""
    resume = _resume_operation()
    coordination = HumanReviewReentryCoordinator.create().resume_and_record(
        resume_operation=resume,
        max_steps=1,
    )
    if coordination.control_plane.digest() != audit_report.control_plane_digest:
        raise AssertionError("test fixture audit report/control plane mismatch")
    return coordination.control_plane
