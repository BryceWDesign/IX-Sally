...
def _state(max_cycles: int = 3) -> NinefoldRunState:
    contract = AutonomyContract.create(
        goal="Expose complete reentry closeout through workflow kit.",
...
def _review_action(
    description: str = "Run workflow-recorded complete reentry closeout.",
) -> BoundedActionRecord:
...
def test_workflow_kit_records_complete_reentry_closeout_operation() -> None:
    ...
    assert operation.require_complete_reentry_closeout_report() == closeout_report
    assert operation.control_plane.complete_reentry_closeout_count() == 1
    assert operation.report.status is (
        HumanReviewControlPlaneReportStatus.COMPLETE_REENTRY_CLOSEOUT_ACCEPTED
    )
    assert operation.report.complete_reentry_closeout_recorded() is True
    assert operation.report.complete_reentry_closeout_accepted() is True
...
def test_workflow_kit_records_waiting_complete_reentry_closeout_status() -> None:
    ...
    assert operation.report.complete_reentry_closeout_recorded() is True
    assert operation.report.complete_reentry_closeout_accepted() is False
    assert operation.report.requires_operator_attention() is False
...
def test_workflow_complete_reentry_closeout_payload_links_report() -> None:
    ...
    assert payload["workflow_stage"] == (
        HumanReviewWorkflowStage.COMPLETE_REENTRY_CLOSEOUT_RECORDED.value
    )
    ...
    assert payload["report_status"] == (
        HumanReviewControlPlaneReportStatus.COMPLETE_REENTRY_CLOSEOUT_ACCEPTED.value
    )
    assert payload["complete_reentry_closeout_count"] == 1
    assert report_payload["latest_complete_reentry_closeout_digest"] == (
        operation.control_plane.latest_complete_reentry_closeout_digest()
    )
    assert report_payload["complete_reentry_closeout_recorded"] is True
